...
export const hsv2hsl = (h: number, s: number, v: number) => {
  return [h, (s * v) / ((h = (2 - s) * v) < 1 ? h : 2 - h) || 0, h / 2]
}

const INT_HEX_MAP: Record<number | string, string> = {
  10: 'A',
  11: 'B',
  12: 'C',
  13: 'D',
...
export const rgb2hex = ({ r, g, b }: any) => {
...
export const rgb2hsv = ({ r, g, b }: any) => {
  r = bound01(r, 255)
  g = bound01(g, 255)
  b = bound01(b, 255)
...
export const hsl2hsv = ({ h, s, l }: any) => {
  s = s / 100
  l = l / 100
  let sMin = s
...
const pickUpRgb = (rgb: string) => {
  const [r, g, b, a]: any[] | any = rgb.match(/(\d(\.\d+)?)+/g)
  return {
    r,
    g,
    b,
...
const pickUpHsl = (hsl: string) => {
  const [h, s, l, a]: Array<any> | any = hsl.match(/(\d(\.\d+)?)+/g)
  return {
    h,
    s: parseFloat(s),
    l: parseFloat(l),
...
const pickUpHsv = (hsv: string) => {
  const [h, s, v, a]: Array<any> | any = hsv.match(/(\d(\.\d+)?)+/g)
  return {
    h: parseFloat(h),
    s: parseFloat(s),
    v: parseFloat(v),
...
export const checkColor = (color: string, format: any, useAlpha = true) => {
  if (useAlpha) {
    switch (format) {
      case 'hex':
      ...
      case 'rgb':
        return color.match(
          /^rgba\((25[0-5]|2[0-4][0-9]|[0-1]?[0-9]?[0-9]),(\s*)(25[0-5]|2[0-4][0-9]|[0-1]?[0-9]?[0-9]),(\s*)(25[0-5]|2[0-4][0-9]|[0-1]?[0-9]?[0-9]),(\s*)(0\.\d{1,2}|1|0)\)/g,
        )
      case 'hsl':
        return color.match(
          /^hsla\((((([0-9]|([1-9][0-9])|([0-2][0-9][0-9])|([3][0-5][0-9])|([0]{1}))|360).[0-9]?[0-9])|(([0-9]|([1-9][0-9])|([0-2][0-9][0-9])|([3][0-5][0-9])|([0]{1}))|360)),(\s*)([0-9]?[0-9]|100)%,(\s*)([0-9]?[0-9]|100)%,(\s*)(0\.\d{1,2}|1|0)\)/g,
        )
    }
  } else {
    switch (format) {
      ...
      case 'rgb':
        return color.match(
          /^rgb\((25[0-5]|2[0-4][0-9]|[0-1]?[0-9]?[0-9]),(\s*)(25[0-5]|2[0-4][0-9]|[0-1]?[0-9]?[0-9]),(\s*)(25[0-5]|2[0-4][0-9]|[0-1]?[0-9]?[0-9])\)/g,
        )
      case 'hsl':
        return color.match(
          /^hsl\((((([0-9]|([1-9][0-9])|([0-2][0-9][0-9])|([3][0-5][0-9])|([0]{1}))|360).[0-9]?[0-9])|(([0-9]|([1-9][0-9])|([0-2][0-9][0-9])|([3][0-5][0-9])|([0]{1}))|360)),(\s*)([0-9]?[0-9]|100)%,(\s*)([0-9]?[0-9]|100)%\)/g,
        )
    }
  }
}

export const checkColorValue = (
  color: string,
  format: Format,
  useAlpha: boolean,
) => {
  if (useAlpha) {
    switch (format) {
      case 'rgb':
        return /^[rR][gG][Bb][Aa][(]([\s]*(2[0-4][0-9]|25[0-5]|[01]?[0-9][0-9]?)[\s]*,){3}[\s]*(1|1.0|0|0.[0-9]|0.[0-9][0-9])[\s]*[)]{1}$/.test(
          color,
        )
      case 'hsv':
        return /^[hH][Ss][Vv][Aa][(]([\s]*(2[0-9][0-9]|360｜3[0-5][0-9]|[01]?[0-9][0-9]?)[\s]*,)([\s]*((100|[0-9][0-9]?)%|0)[\s]*,){2}([\s]*(1|1.0|0|0.[0-9]|0.[0-9][0-9])[\s]*)[)]$/.test(
          color,
        )
      case 'hsl':
        return /^[hH][Ss][Ll][Aa][(]([\s]*(2[0-9][0-9]|360｜3[0-5][0-9]|[01]?[0-9][0-9]?)[\s]*,)([\s]*((100|[0-9][0-9]?)%|0)[\s]*,){2}([\s]*(1|1.0|0|0.[0-9]|0.[0-9][0-9])[\s]*)[)]$/.test(
          color,
        )
      case 'hex':
      default:
    ...
    switch (format) {
      ...
      case 'hsv':
        return /^[hH][Ss][Vv][(]([\s]*(2[0-9][0-9]|360｜3[0-5][0-9]|[01]?[0-9][0-9]?)[\s]*,)([\s]*((100|[0-9][0-9]?)%|0)[\s]*,)([\s]*((100|[0-9][0-9]?)%|0)[\s]*)[)]$/.test(
          color,
        )
      case 'hsl':
...
export const filterHsva = ({
  h,
  s,
  v,
  a,
}: { h: number; s: number; v: number; a: number } | any) => {
...
export const checkHsva = (
  hsva: { h: number; s: number; v: number; a: number } | null,
) => {
